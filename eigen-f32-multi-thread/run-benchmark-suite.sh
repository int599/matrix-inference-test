...
target_path_for_mode() {
  ...
  printf '%s/eigen-f32-multi-thread/eigen_f32_multi_thread_%s\n' "${BUILD_DIR}" "${simd_suffix}"
}

mkdir -p "${REPORT_DIR}"
rm -f "${REPORT_DIR}"/run-*.txt "${REPORT_DIR}"/thread-scaling-report-*.md \
  "${REPORT_DIR}"/thread-scaling-*.png "${REPORT_DIR}"/thread-scaling-data-*.csv \
  "${REPORT_DIR}"/thread-scaling-meta-*.json "${DATA_PATH}"

make -C "${SCRIPT_DIR}" configure >/dev/null

printf "simd_mode\tstatus\tparticipant_count\tmeasured_per_inference_us\tmeasured_total_ms\tcompute_cpus\tuses_hyper_threads\n" > "${DATA_PATH}"

IFS=',' read -r -a thread_counts <<< "${MATRIX_BENCH_THREAD_COUNTS}"
...
for simd_mode in "${simd_modes[@]}"; do
  ...
  if ! matrix_bench_host_supports_simd_mode "${simd_mode}"; then
    ...
    for threads in "${thread_counts[@]}"; do
      printf "%s\tunsupported\t%s\tNA\tNA\tNA\tNA\n" "${simd_mode}" "${threads}" >> "${DATA_PATH}"
    done
    continue
  fi
  ...
  for threads in "${thread_counts[@]}"; do
    ...
    for repeat_idx in $(seq 1 "${REPEATS}"); do
      output="$("${target_path}" -j "${threads}")"
      printf "%s\n" "${output}" > "${REPORT_DIR}/run-${simd_mode}-j${threads}-r${repeat_idx}.txt"
      ...
      candidate_total_ms="$(printf "%s\n" "${output}" | awk -F= '/^measured_total_ms=/{print $2}')"
      if [[ -z "${best_total_ms}" ]] || awk "BEGIN { exit !(${candidate_total_ms} < ${best_total_ms}) }"; then
        best_total_ms="${candidate_total_ms}"
        best_output="${output}"
      fi
    done

    printf "%s\n" "${best_output}" > "${REPORT_DIR}/run-${simd_mode}-j${threads}.txt"

    measured_total_ms="$(printf "%s\n" "${best_output}" | awk -F= '/^measured_total_ms=/{print $2}')"
    measured_per_inference_us="$(printf "%s\n" "${best_output}" | awk -F= '/^measured_per_inference_us=/{print $2}')"
    compute_cpus="$(printf "%s\n" "${best_output}" | awk -F= '/^compute_cpus=/{print $2}')"
    uses_hyper_threads="$(printf '%s\n' "${best_output}" | awk -F= '/^uses_hyper_threads=/{print $2}')"

    printf "%s\tok\t%s\t%s\t%s\t%s\t%s\n" \
      "${simd_mode}" "${threads}" "${measured_per_inference_us}" "${measured_total_ms}" \
      "${compute_cpus}" "${uses_hyper_threads}" >> "${DATA_PATH}"
  done
done

if [[ -z "${suite_matrix_shape}" ]]; then
  printf 'failed to capture multi-thread suite metadata from benchmark output\n' >&2
  exit 1
fi

matrix_bench_tsv_to_csv "${DATA_PATH}" "${CSV_PATH}"
export MATRIX_BENCH_THREAD_COUNTS
...
MATRIX_BENCH_META_SUITE="multi-thread" \
...
thread_counts = [
    int(value)
    for value in os.environ["MATRIX_BENCH_THREAD_COUNTS"].split(",")
    if value
]
...
meta = {
    ...
    "benchmark": {
        ...
        "binding_policy": (
            "Fill physical cores first, then extend to SMT sibling CPUs only "
            "when participant count exceeds physical-core count."
        ),
    },
